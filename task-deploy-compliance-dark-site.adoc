...
summary: 請完成幾個步驟、在內部部署網站上無法存取網際網路的 Linux 主機上安裝 BlueXP 分類。這類安裝最適合安全的站台。 
---
...
[role="lead"]
請完成幾個步驟、在內部部署網站上無法存取網際網路的 Linux 主機上安裝 BlueXP 分類。這類安裝最適合安全的站台。
...
以這種方式安裝（有時稱為「離線」或「暗」站台）時、 BlueXP 分類只能掃描內部部署站台本機資料來源的資料。目前、 BlueXP 分類可以掃描下列 * 本機 * 資料來源：
...
目前不支援掃描 Cloud Volumes ONTAP 、 Azure NetApp Files 、適用於 ONTAP 的 FSX 、 AWS S3 或 Google Drive 、 當 BlueXP 分類部署於黑暗的網站時、 OneDrive 或 SharePoint Online 帳戶。
...
* 從BlueXP自動升級軟體
+
BlueXP Connector 和 BlueXP 分類都需要定期手動升級、才能啟用新功能。您可以在 BlueXP 分類 UI 頁面底部看到 BlueXP 分類版本。請檢查 link:whats-new.html["BlueXP 分類版本資訊"] 查看每個版本的新功能、以及您是否想要這些功能。然後您可以依照下列步驟進行 https://["升級BlueXP Connector"^] 和 <<升級 BlueXP 分類軟體,升級 BlueXP 分類軟體>>。
...
如果您的離線內部部署站台上尚未安裝Connector、 https://["部署Connector"^] 現在就在Linux主機上。
...
如果您的離線內部部署站台尚未安裝BlueXP Connector、 https://["部署Connector"^] 在離線站台的Linux主機上。
...
* 確保 Connector 擁有部署資源的權限、並為 BlueXP 分類執行個體建立安全性群組。您可以在中找到最新的BlueXP權限 https://["NetApp 提供的原則"^]。
...
| Connector <> BlueXP 分類 | 8080（TCP）、443（TCP）及80 | Connector 的安全性群組必須允許透過連接埠 443 進出的流量進出 BlueXP 分類執行個體。請確定連接埠8080已開啟、以便您在BlueXP中查看安裝進度。 
...
.步驟
. 在網際網路設定的系統上、從下載 BlueXP 分類軟體 https://["NetApp 支援網站"^]。您應該選取的檔案名稱為* DataSense-offline bund-<版本>.tar.gz*。
...
.開始之前
* BlueXP 分類軟體可一次升級一個主要版本。例如、如果您已安裝版本 1.2.x 、則只能升級至 1.21.x如果您落後幾個主要版本、就必須多次升級軟體。
* 確認您的 BlueXP Connector 軟體已升級至最新的可用版本。 https://["請參閱連接器升級步驟"^]。
