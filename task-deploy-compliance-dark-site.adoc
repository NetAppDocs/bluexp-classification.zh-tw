...
[role="quick-margin-para"]
BlueXP 分類掃描的前 500 TB 資料可免費使用 30 天。NetApp需要BYOL授權、才能在該點之後繼續掃描資料。BlueXP 核心版本 1.31 免費。
...
您也可以 link:task-licensing-datasense.html["設定 BlueXP 分類的 BYOL 授權"] 目前從 BlueXP 數位錢包頁面。30天免費試用期結束前、您將不需付費。
