...
如果您需要非常安全的Cloud Manager安裝、但又想要從OneDrive帳戶或SharePoint Online帳戶掃描本機資料、則可以使用Data Sense離線安裝程式、並提供網際網路存取功能、存取少數幾個選定的端點。請參閱 <<SharePoint and OneDrive special requirements,SharePoint和OneDrive的特殊需求>> 以取得詳細資料。
