...
| 檔案路徑 | 輸入最多20個要包含或排除在查詢之外的部分或完整路徑。如果您同時輸入「包含路徑」和「排除路徑」、則「Data Sense」會先尋找包含路徑中的所有檔案、然後從排除路徑中移除檔案、然後顯示結果。 
...
| 上次存取 | 選取上次存取檔案的時間範圍。您也可以指定自訂時間範圍、以進一步精簡搜尋結果。對於Data Sense掃描的檔案類型、這是Data Sense掃描檔案的最後一次。 
