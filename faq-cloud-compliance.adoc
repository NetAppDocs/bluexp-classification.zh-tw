...
Cloud Data Sense可在使用者選擇的任何位置（無論是雲端或內部部署）部署單一伺服器或叢集。伺服器透過標準傳輸協定連線至資料來源、並在Elasticsearch叢集中建立結果索引、該叢集也部署在相同的伺服器上。這可支援多雲端、跨雲端、私有雲及內部部署環境。
...
Cloud Manager的實作功能可在所有主要大型超大規模伺服器中作為第一方服務（AWS FSX for NetApp ONTAP 支援AWS；Azure NetApp Files 支援Azure；Cloud Volumes Service 在Google Cloud中支援FSX）。所有這些實作均運用Cloud Manager服務、例如Cloud Data Sense、並可透過Cloud Manager的單一窗口介面、以點選方式啟用功能來進行管理。Cloud Data Sense等服務可由任何使用大型超大規模伺服器的第一方產品之一的授權Cloud Manager使用者無縫啟用。此外、所有的市場都有提供這些服務、而且這些服務是授權與超大規模供應商共同銷售產品、也就是說、如果客戶啟用這些服務、超大規模銷售團隊就能獲得補償。所有這些因素都將Cloud Manager服務分散在整個生態系統中、並擴大我們的服務範圍。

這些第一方產品可讓我們優先存取超大規模市場中數百萬位使用者、這些使用者需要世界級的資料服務、例如NetApp Cloud Data Sense、Cloud Backup、Cloud Tiering、Cloud Sync 物件庫、Global File Cache、 以及更多資訊。使用者在所有市場中、都能以前所未有的輕鬆方式交易及部署服務、以補充及完成其雲端產品組合。
...
Cloud Data Sense服務提供廣泛的搜尋功能、可讓您輕鬆搜尋所有連線來源的特定檔案或資料。Cloud Data Sense讓使用者能夠更深入地搜尋、而不只是中繼資料的反映。這項不受語言限制的服務也能讀取檔案、分析各種敏感資料類型、例如名稱和ID。例如、使用者可以搜尋結構化和非結構化的資料儲存區、找出可能從資料庫洩漏到使用者檔案的資料、違反企業原則。搜尋可儲存以供日後使用、並可建立原則以設定頻率搜尋結果並採取行動。

找到感興趣的檔案後、即可列出特性、包括標記、工作環境帳戶、儲存庫、檔案路徑、 類別（從分類）、檔案大小、上次修改、權限狀態、重複項目、 敏感度層級、個人資料、檔案內的敏感資料類型、擁有者、檔案類型、檔案大小、建立時間、 檔案雜湊、是否將資料指派給尋求注意的人員等。篩選條件可用於篩選出不相關的特性。Cloud Data Sense也具有RBAC控制項、可在有適當權限時、允許移動或刪除檔案。如果沒有適當的權限存在、則可將工作指派給組織中擁有適當權限的人員。



=== Cloud Data合理提供何種分析？

資料來源可以視覺化呈現、並以圖形方式定義和描繪關係。例如、管理員可以查看整個企業內所有資料來源（內部部署系統、資料庫、雲端檔案共用、S3儲存區、OneDrive、 等）。然後、他們可以移動、刪除及管理資料、以最佳化儲存成本並降低風險。使用者可以查看哪些敏感資料可能會外洩、藉此降低風險、並建立工作來管理強大資料保護的權限。Cloud Data Sense服務也會將所有不同類型的資料進行分類、讓管理員可以根據類型來調查資料、查看對資料採取的行動、以及採取的時間。
...
Cloud Manager可讓使用者在幾乎任何位置掃描及報告系統、包括內部部署、雲端和混合式環境。Cloud Data Sense通常是使用SaaS模式來部署、透過Cloud Manager介面來啟用服務、不需要安裝硬體或軟體。即使在這種點選執行部署模式下、資料管理（包括同步、分層、備份與還原）、 無論資料儲存區是內部部署或公有雲、都能實現安全性、控管及法規遵循。

對於安全的安裝、Cloud Manager和Cloud Data Sense可部署在「暗站」模式中、此模式是以套件形式安裝在內部部署環境中、不需要外部網路連線。

無論部署方法為何（內部部署或SaaS交付的雲端模式）、使用者都能完全掌控安裝組態。不會傳送任何資訊給NetApp；完整處理會封裝在伺服器中。
