...
您可以將自訂關鍵字新增至Data Sense、以便識別資料中的資訊位置。只要輸入您想要讓Data Sense辨識的每個字詞、就能新增關鍵字。關鍵字會新增至Data Sense已使用的現有預先定義關鍵字、結果會顯示在「個人模式」區段下方。

例如、您可能想要查看所有檔案中提及內部產品名稱的位置、以確保這些名稱在不安全的位置無法存取。

更新自訂關鍵字之後、Data Sense會重新開始掃描所有資料來源。掃描完成後、新結果會顯示在「個人結果」區段下方的「資料感知法規遵循儀表板」、以及「個人資料」篩選器的「調查」頁面。
...
+
您輸入的名稱會顯示在「Data Sense UI」中、做為符合類別器需求的掃描檔案標題、以及「Investigation」（調查）頁面中的篩選名稱。
...
+
image:screenshot_select_classifier_tool_keywords.png["快照顯示「自訂關鍵字」選項、做為Data Sense用來建置模式的工具。"]

. 在_建立邏輯_頁面中、輸入您要辨識的關鍵字-每個字位於一行、然後按一下*驗證*。
+
以下螢幕快照顯示內部產品名稱（不同類型的貓頭鷹）。資料感應搜尋這些項目不區分大小寫。

+
image:screenshot_select_classifier_create_logic_keyword.png["輸入客戶分類器關鍵字的快照。"]

. 按一下「*完成*」、「Data Sense（資料感知）」即開始重新掃描資料。
...
您可以使用自訂規則運算式（regex）新增個人模式、以識別資料中的特定資訊。這可讓您建立新的自訂regex、以識別系統中尚未存在的新個人資訊元素。regex會新增至Data Sense已使用的現有預先定義模式、結果會顯示在「個人模式」區段下方。

例如、您可能想要查看所有檔案中提及內部產品ID的位置。例如、如果產品ID的結構清楚明確、12位數的編號從201開始、您可以使用自訂的regex功能在檔案中搜尋。此範例的規則運算式為*\b201\d｛9｝\b*。

新增regex之後、Data Sense會重新開始掃描所有資料來源。掃描完成後、新結果會顯示在「個人結果」區段下方的「資料感知法規遵循儀表板」、以及「個人資料」篩選器的「調查」頁面。
...
. 在_選取類型_頁面中、輸入類別器的名稱、提供簡短說明、選取*個人識別符號*、然後按*下一步*。
+
您輸入的名稱會顯示在「Data Sense UI」中、做為符合類別器需求的掃描檔案標題、以及「Investigation」（調查）頁面中的篩選名稱。您也可以勾選「遮罩系統中偵測到的結果」方塊、使完整結果不會出現在UI中。例如、您可能想要隱藏完整的信用卡號碼或類似的個人資料。
...
+
image:screenshot_select_classifier_tool_regex.png["快照顯示「自訂規則運算式」選項、做為Data Sense建置模式所用的工具。"]

. 在_建立邏輯_頁面中、輸入規則運算式和任何鄰近字詞、然後按一下*完成*。
+
.. 您可以輸入任何合法的規則運算式。按一下「*驗證*」按鈕、讓「資料感知」驗證規則運算式是否有效、且其範圍不太廣、表示會傳回太多結果。
...
.結果
新增類別器、Data Sense開始重新掃描所有資料來源。您會回到「自訂類別器」頁面、在其中可以檢視符合新類別器的檔案數量。掃描所有資料來源所產生的結果、將會花費一些時間、視需要掃描的檔案數量而定。
...
Data Sense會取得掃描的資料、並將其分成不同類別。類別是以人工智慧分析每個檔案的內容和中繼資料為基礎的主題。 link:reference-private-data-categories.html#types-of-categories["請參閱預先定義類別清單"]。

類別可顯示您擁有的資訊類型、協助您瞭解資料的現況。例如、像是_恢復_或_員工合約_的類別可能包含敏感資料。調查結果時、您可能會發現員工合約儲存在不安全的位置。然後您就可以修正該問題。

您可以將自訂類別新增至「Data Sense」（資料感測）、以便識別資料中的資料資產專屬資訊類別。您可以建立包含您要識別之資料類別的「訓練」檔案來新增每個類別、然後讓Data Sense掃描這些檔案以透過AI「學習」、以便識別資料來源中的資料。類別會新增至Data Sense已識別的現有預先定義類別、結果會顯示在「類別」區段下方。

例如、您可能想要查看.gz格式的壓縮安裝檔案在檔案中的位置、以便視需要加以移除。

更新自訂類別之後、Data Sense會重新開始掃描所有資料來源。掃描完成後、新的結果會顯示在「類別」區段下的「資料感知法規遵循儀表板」、以及「類別」篩選器的「調查」頁面。 link:task-controlling-private-data.html#viewing-files-by-categories["瞭解如何依類別檢視檔案"]。

.您需要的產品
您至少需要建立25個訓練檔案、其中包含您希望Data Sense能夠辨識的資料類別範例。支援下列檔案類型：

`+.CSV, .DOC, .DOCX, .GZ, .JSON, .PDF, .PPTX, .RTF, .TXT, .XLS, .XLSX, Docs, Sheets, and Slides+`

檔案必須至少100個位元組、而且必須位於Data Sense可存取的資料夾中。
...
+
您輸入的名稱會顯示在「Data Sense UI」中、做為符合您所定義資料類別的掃描檔案標題、以及「Investigation」（調查）頁面中的篩選名稱。
...
+
image:screenshot_category_create_logic.png["「建立邏輯」頁面的快照、您可在其中新增包含Data Sense所需學習之資料的檔案。"]

. 輸入磁碟區的IP位址、以及訓練檔案所在的路徑、然後按一下*「Add*（新增*）」。
+
image:screenshot_category_add_files.png["顯示如何輸入訓練檔案位置的快照。"]

. 確認Data Sense已辨識訓練檔案。按一下* x*以移除任何不符合要求的訓練檔案。然後按一下「*完成*」。
+
image:screenshot_category_files_added.png["快照顯示Data Sense將用來做為定義新類別的訓練檔案的檔案。"]



.結果
新類別是依照訓練檔案所定義、並新增至Data Sense。然後、Data Sense會開始重新掃描所有資料來源、以識別適合此新類別的檔案。您會返回「自訂分類器」頁面、在其中可以檢視符合新類別的檔案數量。掃描所有資料來源所產生的結果、將會花費一些時間、視需要掃描的檔案數量而定。
...
如果您稍後決定不需要「資料感知」來識別您新增的自訂模式、您可以使用*刪除分類器*按鈕來移除每個項目。
