...
.步驟
. 從「自訂類別器」索引標籤中、按一下「*新增類別器*」以啟動「新增自訂類別器_」精靈。
+
image:screenshot_compliance_add_classifier_button.png["顯示如何啟動「新增自訂類別器」精靈的快照。"]

. 在_選取類型_頁面中、輸入類別器的名稱、提供簡短說明、選取*個人識別符號*、然後按*下一步*。
+
您輸入的名稱會顯示在Data Sense UI中、做為符合類別器需求的掃描檔案標題。您也可以勾選「遮罩系統中偵測到的結果」方塊、使完整結果不會出現在UI中。例如、您可能想要隱藏完整的信用卡號碼或類似的個人資料。
...
+
image:screenshot_select_classifier_tool.png["快照顯示「自訂規則運算式」選項、做為Data Sense建置模式所用的工具。"]
...
+
image:screenshot_select_classifier_create_logic.png["為您的客戶類別者輸入regex和近接字的快照。"]
