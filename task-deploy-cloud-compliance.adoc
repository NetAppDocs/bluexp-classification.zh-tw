...
如果您還沒有連接器、請立即建立連接器。請參閱 https://["在 AWS 中建立連接器"^]、 https://["在 Azure 中建立 Connector"^]或 https://["在GCP中建立連接器"^]。

[role="quick-margin-para"]
您也可以 https://["在內部部署安裝連接器"^] 在您網路中的 Linux 主機或雲端中的 Linux 主機上。
...
如果您還沒有Connector、請在雲端供應商中建立Connector。請參閱 https://["在 AWS 中建立連接器"^] 或 https://["在 Azure 中建立 Connector"^]或 https://["在GCP中建立連接器"^]。在大多數情況下、您可能會在嘗試啟動 BlueXP 分類之前先設定 Connector 、因為大部分的原因 https://["BlueXP功能需要連接器"]但有些情況下您需要立即設定一個。
...
請注意、您也可以 https://["在內部部署安裝連接器"^] 在網路或雲端的Linux主機上。有些打算在內部部署上安裝 BlueXP 分類的使用者也可以選擇在內部部署上安裝 Connector 。
...
https://["如需在政府區域部署 Connector 的詳細資訊、請參閱"^]。
...
--
====
確認BlueXP擁有必要的權限:: 確保 BlueXP 具有部署資源和建立 BlueXP 分類執行個體安全性群組的權限。您可以在中找到最新的BlueXP權限 https://["NetApp 提供的原則"^]。
...
Azure和Azure政府部署需要額外的傳入和傳出安全性群組規則。請參閱 https://["Azure 中的 Connector 規則"^] 以取得詳細資料。
...
* https://["AWS文件：Amazon EC2服務配額"^]
* https://["Azure 文件：虛擬機器 vCPU 配額"^]
