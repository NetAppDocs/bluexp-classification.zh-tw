...
[role="quick-margin-para"]
BlueXP 分類掃描的前 500 TB 資料可免費使用 30 天。您必須透過雲端供應商Marketplace訂閱BlueXP、或取得NetApp BYOL授權、才能在該時間點之後繼續掃描資料。
...
使用這些雲端連接器時、可以掃描內部部署 ONTAP 系統、非 NetApp 檔案共用、一般 S3 物件儲存設備和資料庫。
