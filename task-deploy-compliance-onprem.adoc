...
[role="quick-margin-para"]
確保您的環境符合先決條件。這包括執行個體的傳出網際網路存取、連接器與Cloud Data Sense之間透過連接埠443的連線等。  Cloud Manager and Data Sense prerequisites,請參閱完整清單。
...
|===
--
確保 Cloud Manager 擁有所需的權限:: 確保Cloud Manager擁有部署資源的權限、並為Cloud Data Sense執行個體建立安全群組。您可以在中找到最新的 Cloud Manager 權限 https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html["NetApp 提供的原則"^]。
...
請參閱  the Linux host system,準備Linux主機系統 和  Cloud Manager and Data Sense prerequisites,檢閱先決條件 以取得部署Cloud Data Sense之前的完整需求清單。
...
* 確認您的離線環境符合所需  Cloud Manager and Data Sense prerequisites,權限與連線能力。
...
* 請確定您擁有Linux系統的root權限。
* 確認您的環境符合所需  Cloud Manager and Data Sense prerequisites,權限與連線能力。
