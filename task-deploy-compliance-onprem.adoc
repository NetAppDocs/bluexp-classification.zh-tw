...
對於掃描PB資料的大型組態、您可以納入多個主機、以提供額外的處理能力。使用多個主機系統時、主要系統稱為Manager節點、而提供額外處理能力的其他系統則稱為掃描器節點。
...
[role="quick-margin-para"]
確保您的環境符合先決條件。這包括執行個體的傳出網際網路存取、連接器與Cloud Data Sense之間透過連接埠443的連線等。  BlueXP and Data Sense prerequisites,請參閱完整清單。

[role="quick-margin-para"]
您也需要符合的Linux系統  the Linux host system,符合下列需求。
...
確保BlueXP Connector能夠存取Cloud Data Sense:: 確保Connector與Cloud Data Sense執行個體之間的連線能力。連接器的安全性群組必須允許傳入和傳出經由連接埠443的流量進出Data Sense執行個體。
+
--
...
對於一般組態、您將在單一主機系統上安裝軟體。  installation for typical configurations,請參閱此處的步驟。

image:diagram_deploy_onprem_single_host_internet.png["圖表顯示使用內部部署且可存取網際網路的單一Data Sense執行個體時、您可以掃描的資料來源位置。"]

對於掃描PB資料的大型組態、您可以納入多個主機、以提供額外的處理能力。  installation for large configurations,請參閱此處的步驟。

image:diagram_deploy_onprem_multi_host_internet.png["圖表顯示當使用部署在內部部署且可存取網際網路的多個Data Sense執行個體時、您可以掃描的資料來源位置。"]

請參閱  the Linux host system,準備Linux主機系統 和  BlueXP and Data Sense prerequisites,檢閱先決條件 以取得部署Cloud Data Sense之前的完整需求清單。
...
如果您有特定的資料來源需要由實體靠近資料來源的掃描儀節點進行掃描、您可以定義掃描儀節點或掃描儀節點群組、以掃描特定的資料來源或資料來源群組。在下圖中、有一個Manager節點和3個掃描儀節點。
...
. 請依照步驟在掃描儀節點上部署軟體（並選擇性地為每個掃描儀節點定義「掃描儀群組」）。
. 如果您定義了掃描器群組、請在Manager節點上：
+
.. 開啟檔案「jobing_for_banner_group_config.yml」、並定義每個掃描器群組要掃描的工作環境
.. 執行下列指令碼、將此對應資訊登錄至所有掃描器節點：「update_we掃描儀_群組_from _config_file.sh」




.您需要的是 #8217 ；需要的是什麼
* 請確認適用於掃描儀節點的所有Linux系統均符合  the Linux host system,主機需求。
* （選用）確認系統已安裝兩個必要的軟體套件（Docker Engine和Python 3）。如果系統上尚未安裝此軟體、安裝程式將會安裝此軟體。
* 請確定您擁有Linux系統的root權限。
* 確認您的環境符合所需  BlueXP and Data Sense prerequisites,權限與連線能力。
* 您必須擁有要新增的掃描儀節點主機的IP位址。
* 您必須擁有Data Sense Manager節點主機系統的IP位址
* 您必須擁有連接器系統的IP位址或主機名稱、NetApp帳戶ID、連接器用戶端ID和使用者存取權杖。請參閱下方的_Prerequisite步驟_以取得此資訊。
* 您必須知道帳戶中每個資料來源的_工作環境ID_。如有您的要求、請聯絡NetApp：ng-contact-data-sense@netapp.com。
...
.. 解壓縮安裝程式檔案。
.. 貼上並執行您在步驟2中複製的命令。
.. 如果您想要將掃描器節點新增至「掃描器群組」、請將參數「-r <掃描 儀群組名稱>」新增至命令。否則掃描儀節點會新增至「預設」群組。
+
...
+
.. 開啟檔案「/opt/netapp/Datasense/siting_Environ_to _bANer_Group_config.yml」、然後輸入掃描程式群組掃描特定工作環境的對應。您必須取得NetApp每個資料來源的_工作環境ID_（電子郵件：ng-contact-data-sense@netapp.com）。例如、下列項目可將2個工作環境新增至2個掃描器群組：
+
....
scanner group:
 europe:
...
....
+
任何未新增至清單的工作環境都會由「預設」群組掃描、您必須在「預設」群組中至少有一個掃描器節點。
...
.步驟
. 請依照中的步驟1至7進行  installation for typical configurations,單一主機安裝 在管理器節點上。
