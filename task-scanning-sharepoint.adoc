...
* 對於SharePoint Online而言、Data Sense可以是 link:task-deploy-cloud-compliance.html["部署於雲端"^]。
...
如果在沒有網際網路存取的網站上安裝Data Sense、則同樣也必須在沒有網際網路存取的同一個網站上安裝BlueXP Connector。 https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-private-mode.html["深入瞭解"^]。
