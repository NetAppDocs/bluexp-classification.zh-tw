...
您可以 https://["在內部部署安裝連接器"^] 在您網路中的 Linux 主機或雲端中的 Linux 主機上。有些打算在內部部署上安裝 BlueXP 分類的使用者也可以選擇在內部部署上安裝 Connector 。

若要在雲端供應商環境中建立 Connector 、請參閱 https://["在 AWS 中建立連接器"^]、 https://["在 Azure 中建立 Connector"^]或 https://["在GCP中建立連接器"^]。
...
.步驟
. 從下載 BlueXP 分類先決條件指令碼 https://["NetApp 支援網站"^]。您應該選擇的檔案名稱為 * 獨立式 - 必要的 - 測試者 - <version> * 。
